...
from common.all_path import wkdl_sr_file_path, wkdl_sequencing_group_number, wkdl_non_sr_file_path, \
    ...
    wkdl_result_non_sr_import_path, wkdl_result_sr_import_path, wkdl_result_standard_sample_path
...
class LibraryQuantificationPage(BasePage):
    ...
    def check_lims_num(self):
        ...
        non_sr_lims = self.get_non_sr_sample_from_excel('富集lims号')
    ...
    # 准备非SR类型批量粘贴导入的数据
    def read_non_sr_import_data(self, non_sr_samples, import_file_path, enrichments_name, Sequencing_group_number):
        """
          根据非sr样本数量，修改非sr类型导入模板
        :param non_sr_samples: Excel读取非SR样本
        :param import_file_path: 导入模板路径
        :param enrichments_name:系统中，富集名称的定位
        ...
        for g in range(0, lims_count[0]):
            k = g + 1
            # 依次取出明细表中富集名称，与Excel流转数据中的富集名称进行比对，如果与流转表中的一致则为非sr样本
            lims_num = self.get_text('css', enrichments_name.format(k))
            if lims_num in non_sr_samples:
                self.sleep(0.5)
        ...
        data = xlrd.open_workbook(import_file_path)
        num_list = []
        for nss in range(0, len(non_sr_samples)):
            h = nss + 1
            tables = data.sheets()[0]
            vals = tables.row_values(h)
            imp_data = '\t'.join(map(str, vals))
    ...
    def detail_libraryenrichment(self):
        ...
        non_sr_samples = self.get_non_sr_sample_from_excel('富集名称')  # 获取非SR样本数量
    ...
    def create_result_and_sequencing_group_number(self):
        ...
        lims_nub = read_excel_col(wkdl_non_sr_file_path, "富集名称")
    ...
    def single_gradient_quantification(self):
        ...
        print(task_id)
        sql = wkdl_detail_sql2.format(task_id[5:].strip())
        self.updata_sql(sql)
        # 刷新页面
        self.refresh()
        self.wait_loading()
    ...
    def serach_task(self):
        ...
        lims_id = read_excel_col(wkdl_non_sr_file_path, '富集名称')
        enrichment_nub = read_excel_col(wkdl_sr_file_path, 'lims号')
        ...
        if lims_id:
            ...
            self.input('css', search_lims_sample_num, enrichment_nub[0])
        self.sleep(1)
